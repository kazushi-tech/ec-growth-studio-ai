...
- **目的**: 上司・経営層に「BigQuery に繋がっている」「数字が動く」を見せ、
  Phase 3 / 4 の予算と時間の合意を取る
...
- **完了条件**:
  - Dashboard 上部の「BigQuery 接続」トグル ON で、売上 / 注文数 / AOV が BQ 値に置換
  - データソース Pill が `BigQuery` (緑) に切り替わる
...
- **Pill ソースラベル**: `BigQuery (live)` / `CSV取込` / `サンプル` のいずれか（§7参照）
...
| --- | --- | --- | --- |
| BigQuery 接続成功 | BQ live data | `BigQuery (live)` (緑) | （不要） |
| BigQuery 接続失敗（タイムアウト / 5xx） | CSV 取込値 → 無ければサンプル | 失敗をユーザーに **明示**（赤バナー or トースト）したうえで `CSV取込` または `サンプル` Pill | エラーバナーは閉じれる。CSV があれば自動でフォールバック |
| BigQuery トグル OFF / 未設定 | CSV 取込値 → 無ければサンプル | `CSV取込` または `サンプル` | (Phase 3A 未実装時の既定動作) |
| CSV 取込済み + BQ 未接続 | CSV 取込値 | `CSV取込` (青系) | 既存 Phase 2.5 と同じ |
| CSV 未取込 + BQ 未接続 | サンプル | `サンプル` (灰) | 既存と同じ |
| 「サンプルデータに戻す」操作後 | サンプル | `サンプル` | 既存と同じ。BQ トグルは OFF に戻す |
...
Phase 3A で UI 実装を行う場合の **最小 UI**。今回は実装しないが、設計レビュー用に列挙する。

### 7-1. 候補要素

| 要素 | 役割 | 必須度 |
| --- | --- | --- |
| Dashboard 上部「BigQuery 接続」トグル | BQ 値と CSV / サンプル値の切替 | 必須 |
| データソース Pill（`BigQuery` / `CSV取込` / `サンプル`） | 現在表示中のソースを明示 | 必須 |
...
- [`src/pages/Dashboard.tsx`](../src/pages/Dashboard.tsx) — KPI カード上部に切替 UI / Pill / バナー
- [`src/components/ui/Pill.tsx`](../src/components/ui/Pill.tsx) — `live` / `csv` / `sample` の tone 追加
  （ファイル名は仮、実在は実装時に確認）
- [`src/lib/data-source/`](../src/lib/) — 新設想定。BQ / CSV / サンプルの優先順位ロジック
...
**まだ実装しない前提**で、次に実装するならどのファイル / ディレクトリを触るかの一覧。

### 8-1. 新規作成（API側）

- `api/bq/health.ts` または `api/bq/health.js`
  — Vercel Functions エンドポイント。BQ 接続確認用（`SELECT 1` 相当）
- `api/bq/orders-daily.ts`
  — `orders_daily` テーブルから期間指定で取得
- `api/bq/orders-by-product.ts`
  — `orders_by_product` テーブルから期間指定で取得（Phase 3A スコープに含めるかは要判断）

### 8-2. 新規作成（共通ロジック）

- `src/lib/bq/client.ts`
  — フロント側 fetch ラッパー（タイムアウト / リトライ / SWR キャッシュ）
- `src/lib/bq/types.ts`
  — `BqOrdersDaily` / `BqOrdersByProduct` の DTO 型
- `src/lib/data-source/index.ts` (新規ディレクトリ)
  — BQ / CSV / サンプルの優先順位ロジック / `useDataSource()` フック
...
- [`src/pages/Dashboard.tsx`](../src/pages/Dashboard.tsx) — BQ トグル / Pill / バナー追加。
  KPI 値を `useDataSource()` 経由に切り替え
- [`src/components/ui/Pill.tsx`](../src/components/ui/Pill.tsx) — tone マップ拡張
  （ファイル名は実装時に確認）
- [`src/lib/csv/ImportContext.tsx`](../src/lib/csv/ImportContext.tsx) — DataSourceContext と統合
  （§8-5 参照）

### 8-4. 設定・ドキュメント

- `.env.example` — `GCP_PROJECT_ID` / `BQ_DATASET` / `ALLOWED_ORIGINS` / `MAX_QUERY_DAYS`
  の **キーのみ** 記載（値は空 / 例示）
...
→ **推奨**: 新しい `DataSourceContext` を新設し、`ImportContext` は CSV 取込状態
  だけを引き続き持つ。`useDataSource()` フックが BQ / CSV / サンプルの優先順位判定を担う。
