...
- 分析に必要なデータ: 注文CSV（取込済み）/ GA4 CSV（次フェーズ）/ 広告CSV（次フェーズ）/ BigQuery（将来・読み取り）
...
- GA4 / 広告 / BigQuery の実データ反映（ここを差し替えるとリアルタイム化する）
- 影響額の厳密な対数分解（現状は近似表示）
...
- DB永続化（Supabase 等）、商品CSV / 広告CSV / 在庫CSV、AI診断 API、Shopify API は次フェーズ
...
GA4 CSV 取込で「セッション × CVR × AOV」が実値化したら、次フェーズで
広告CSVを足してチャネル別 ROAS / CVR を組み合わせ、その後 BigQuery 直接接続で
自動更新化する想定。
...
- **Phase 2（現在）**: GA4 CSVエクスポートを取り込み、セッション × CVR × AOV を半実データ化
- **Phase 2.5（次候補）**: 広告CSVを取り込み、チャネル別 ROAS / CVR / 広告費効率を要因分解に接続
- **Phase 3**: BigQuery 読み取り接続。Insight Studio で構築済みの BigQuery分析基盤を、
  EC向けの売上要因分解にそのまま接続して自動更新化する
- **Phase 4**: GA4 Data API / 広告API の直接連携、AI診断ロジック、永続化、PDF出力

## 5. 未実装範囲（プロトタイプ時点）

- 注文CSV / GA4 CSV 以外（商品 / 広告 / 在庫 / レビュー / CRM）のパース・マッピング
- 取込結果のDB永続化（現在はブラウザ内 `localStorage` のみ）
- Shopify / GA4 / 広告 / BigQuery API 接続（OAuth / GCP認証 / クエリ実行を含む）
