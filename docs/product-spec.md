...
- **Phase 3A（着手中）**: BigQuery health endpoint。Vercel Functions で
  [`api/bq/health.ts`](../api/bq/health.ts) を実装し、サーバー側から `SELECT 1` と
  dataset 存在確認のみを行う最小構成。Dashboard UI / `/api/bq/orders-daily` /
  CSV fallback ロジックは未実装で、後続 PR で対応する。
