...
### 現在の接続状態（実値 / デモ / 未接続 / 将来予定）

UI上の各画面では、表示される数値が「どのデータソース由来か」を Pill / バナー / 凡例で
明示している。本MVPで担保しているのは以下の4分類のみで、**実 GCP / 実 API / 実 AI には未接続**:

| 区分 | 対象 | 状態 |
| --- | --- | --- |
| 実値（CSVで動作中） | 注文 / GA4 / 広告 CSV | ブラウザ内 localStorage で取込、売上 / 注文 / AOV / セッション / CVR / ROAS / CPC / 効率悪化キャンペーン まで実値で計算 |
| デモ（接続後の再現 / Previewのみ） | BigQuery デモ Mode | `BQ_MOCK_MODE=true` の Preview のとき `/api/bq/*` が `mode:"mock"` を返す。**実 GCP 接続ではない**。Production は `BQ_MOCK_MODE` 未設定なので `501 / NOT_IMPLEMENTED` で安全停止し、KPIは CSV/サンプル値にフォールバック |
| 未接続（次フェーズ） | 実 BigQuery クエリ実行 | Phase 3 後続 PR で実装予定（GCP プロジェクト / dataset / サービスアカウント / 課金 / Budget Alert は **未設定**） |
| 未接続（次フェーズ） | GA4 Data API / Google広告 / Meta広告 / Shopify Admin API | Phase 3 / 4 で読み取り専用接続予定 |
| サンプル文言 | AI考察レポート / 主因候補ヘッドライン | 静的な参考文。Phase 4 で Anthropic SDK + prompt cache に接続予定 |
| 未接続（次フェーズ） | 認証 / 永続化 / マルチテナント | Phase 4 で Supabase + Edge Functions / 認証実装予定 |

UIでは以下のラベル・凡例を共通で使い、上司デモでも誤解されないようにする:

- `注文CSV実値` / `GA4実値` / `広告CSV実値` — CSV取込 由来の実値
- `推定値` — 静的サンプル（前月分などCSV未対応の値）
- `BigQueryデモ` — `mode:"mock"` の固定 response（実 GCP 接続ではない）
- `サンプル文言` — AI生成テキストは Phase 4 で実 AI 接続するまで固定文
...
### 3.2 ダッシュボード (`/app`)
- 上部: GA4風の操作帯（データソース / CSV状態 / BigQueryデモ / 期間 / 主要CTA）
- KPI帯: 6つのKPIカード（売上・注文・AOV・CVR・リピート率・広告ROAS）+ Sparkline
- First viewport: 「今月の状態」/「AI月次診断」/「次に実行」で、状態 → 異常/機会 → 次アクションを即読できる構成
- 月次改善ワークベンチ: 優先施策 / 商品 / データ状態 / インサイト のタブで、PriorityMap・TOP5・商品別判断・データ状態を切替表示
- データ状態は実値 / デモ / 未接続 / サンプルをPillで明示し、詳細はワークベンチ内に集約する

### 3.2.1 ガイド (`/app/guide`)
- 立ち位置: 利用者向けオンボーディングではなく、管理画面内の操作ガイド / データ状態早見表
- 上部: 目的 / まずやること / データ状態 / 注意点をコンパクトなサマリー帯で表示
- Main: Overview / 操作順 / 画面別 / データ状態 / FAQ のタブ式ガイド
- 常時表示: 3ステップ、画面別早見表、データ状態4区分
- 補足: FAQ・注意事項・画面別詳細は details/summary で折りたたむ
- 上司デモ本文は置かず、`docs/demo-script.md` への参照カードのみ残す
...
- **Phase 3A（着手中 / BigQuery デモ Mode 実装済み）**: GCP 未接続でも上司デモ動線を確認できる
  「BigQuery デモ Mode」を実装。`BQ_MOCK_MODE=true` のとき
  [`api/bq/health.ts`](../api/bq/health.ts) と [`api/bq/orders-daily.ts`](../api/bq/orders-daily.ts) は
  GCP に接続せず `mode:"mock"` の固定 response を返す。Dashboard 上部に
  「BigQueryデモ」トグルとデータソース Pill（BigQueryデモ / CSV取込 / サンプル）を追加し、
  ON で mock summary が 売上 / 注文数 / AOV の KPI に流れる。**実 GCP 接続 / dataset /
  サービスアカウント / 課金 / Budget Alert は未設定**。実 BigQuery への
  orders-daily クエリは Phase 3A 後続 PR で実装予定。
  **Production は `BQ_MOCK_MODE` を設定しない方針** のため、Production でトグルを ON にしても
  `/api/bq/orders-daily` は `501 / NOT_IMPLEMENTED` で安全停止し、KPIは CSV/サンプル値にフォールバックする
  （誤って実GCPに繋がない設計）。BigQueryデモを上司に見せる場合は `BQ_MOCK_MODE=true` の Preview URL を使う。
- **Phase 3（未着手 / 設計案あり）**: BigQuery 読み取り接続。Insight Studio で構築済みの BigQuery分析基盤を、
  ...
  Phase 3A 着手前の意思決定項目は [`docs/phase3a-decision-sheet.md`](phase3a-decision-sheet.md) を参照
